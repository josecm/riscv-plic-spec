...
- *Virtual Interrupt Guest Block ID registers:* +
   The virtual interrupt block id of each guest. +
...
| *PLIC Register Block Name* | *Function*|*Register Block Size in Byte*| *Description*
|Virtual Interrupt Injection Registers
|Virtual Interrupt Injection Registers from Block #0 to #63 for 15872 contexts
|4096 (0x001000) bytes for each block
|This is a continuously memory block contains PLIC Virtual Interrupt Pending Bits for each guest. Each Injection Registers occupies 32-bit from this register block.
|===
...
Each guest context has one register to specify the block source for virtual interrupt injection.

Setting this field `zero` indicates that no injection block is attached to the guest context.

[cols="15%,20%,20%,45%"]
|===
| *PLIC Register Block Name* | *Function*|*Register Block Size in Byte*| *Description*
|Virtual Context Block ID Register
|Virtual Context Block ID Register for each virtual contexts
|(4) * 15872 = 63488(0xf800) bytes
|This is the Virtual Context Block ID Register for each guest.
|===
...
== VirtualInterrupt Claim/Complete Process

When a claim is performed, the `inFlight` bit is set indicating that the interrupt is active.
When a complete is performed, the interrupt `int_id` field is set and the `inFlight` bit is cleared. 
