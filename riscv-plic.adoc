...
- *Virtual Interrupt Pending Bits registers:* +
   The virtual interrupt pending status of each interrupt source for each guest. +
...
- *Virtual Interrupt Priority registers:* +
   The virtual interrupt priority of each interrupt source for each guest. +
...
*PLIC Memory Map*
	...
	base + 0x3FFFFFC: Reserved	
	...
	base + 0x400000: Reserved (virtual interrupt source 0 does not exist) context 0
	base + 0x400004: Virtual Interrupt source 1 priority context 0
	base + 0x400008: Virtual Interrupt source 2 priority context 0
	...
	base + 0x400FFC: Virtual Interrupt source 1023 priority context  0
	base + 0x401000: Virtual Interrupt Pending bit 0-31 context 0
	base + 0x40107C: Virtual Interrupt Pending bit 992-1023 context 0
	base + 0x402000: Virtual Interrupt Type bit 0-31 context 0
	base + 0x40207C: Virtual Interrupt Type bit 992-1023 context 0
	...
	base + 0x403000: Reserved (virtual interrupt source 0 does not exist) context 0
	base + 0x403004: Virtual Interrupt source 1 priority context 1
	base + 0x403008: Virtual Interrupt source 2 priority context 1
	...
	base + 0x403FFC: Virtual Interrupt source 1023 priority context  1
	base + 0x404000: Virtual Interrupt Pending bit 0-31 context 1
	base + 0x40407C: Virtual Interrupt Pending bit 992-1023 context 1
	base + 0x405000: Virtual Interrupt Type bit 0-31 context 1
	base + 0x40507C: Virtual Interrupt Type bit 992-1023 context 1
	...
	base + 0x41FF00: Reserved (virtual interrupt source 0 does not exist) context 0
	base + 0x41FF04: Virtual Interrupt source 1 priority context 15871
	base + 0x41FF08: Virtual Interrupt source 2 priority context 15871
	...
	base + 0x41FFFC: Virtual Interrupt source 1023 priority context  15871
	base + 0x420F00: Virtual Interrupt Pending bit 0-31 context  15871
	base + 0x420FFC: Virtual Interrupt Pending bit 992-1023 context  15871
	base + 0x421000: Virtual Interrupt Type bit 0-31 context  15871
	base + 0x421FFC: Virtual Interrupt Type bit 992-1023 context  15871
	...	
...
PLIC provides a mechanism to inject pure virtual interrupts into the guests running at VS-level, by writting to virtual interrupt pending bits register. This reduces the need to trap and emulate PLIC acesses as the `claim/complete` process is done directly by the guest. The virtual pending bits follows the same structure and size as the normal pending bits array.
When a read is performed, the output will be the logical OR of the `physical pending` and the logical AND of guest source `virtual type bit` with the guest source `virtual pending bit`. 

The virtual pending bits can only affect the guest external lines interrupts state.

A pending bit in the PLIC core can be cleared by setting the associated enable bit and type bit and then performing a claim.

[cols="15%,20%,20%,45%"]
|===
| *PLIC Register Block Name* | *Function*|*Register Block Size in Byte*| *Description*
|Virtual Interrupt Pending Bits
|Virtual Interrupt Pending Bit of Interrupt Source #0 to #N for 15872 contexts
|(1024 / 8) * 15872 = 2031616(0x1f0000) bytes
...
*PLIC Virtual Interrupt Pending Bits Memory Map* +

	0x401000: Virtual Interrupt Pending bit 0-31 context 0
	0x40107C: Virtual Interrupt Pending bit 992-1023 context 0
	0x404000: Virtual Interrupt Pending bit 0-31 context 1
	0x40407C: Virtual Interrupt Pending bit 992-1023 context 1
	...
	...
	...
	0x420F00: Virtual Interrupt Pending bit 0-31 context  15871
	0x420FFC: Virtual Interrupt Pending bit 992-1023 context  15871
...
== Virtual Interrupt Priorities Register 

If PLIC supports virtual Interrupt Priorities, then each PLIC interrupt source for each guest can be assigned a priority by writing to its 32-bit
memory-mapped `virtual priority` register. It behaviour is identical to the normal priority register, but it affects the virtual interrupt delivery and is programmed by the guest.

Each context has the possibility to include the 'virtual priorities registers'.

[cols="15%,20%,20%,45%"]
|===
| *PLIC Register Block Name*| *Function*|*Register Block Size in Byte*| *Description*
|Virtual Interrupt Source Priority
|Virtual Interrupt Source Priority #0 to #1023
|(1024 * 4) * 15871 = 65011712(0x3e00000) bytes
|This is a continuously memory block which contains PLIC Interrupt Source Priority. Total 1024 Virtual Interrupt Source Priority 
in this memory block for each guest context. Interrupt Source Priority #0 is reserved which indicates it does not exist.
|===

*PLIC Interrupt Source Priority Memory Map* +

	0x400004: Virtual Interrupt source 1 priority context 0
	0x400008: Virtual Interrupt source 2 priority context 0
	...
	0x400FFC: Virtual Interrupt source 1023 priority context  0
	0x403004: Virtual Interrupt source 1 priority context 1
	0x403008: Virtual Interrupt source 2 priority context 1
	...
	0x403FFC: Virtual Interrupt source 1023 priority context  1
	...
	...
	...
	0x41FF04: Virtual Interrupt source 1 priority context 15871
	0x41FF08: Virtual Interrupt source 2 priority context 15871
	...
	0x41FFFC: Virtual Interrupt source 1023 priority context  15871
