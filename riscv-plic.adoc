...
- *Virtual Interrupt Injection Status Registers* +
   The virtual interrupt injection status registers of virtual each interrupt injection block. +

- *Virtual Interrupt Injection Info Registers* +
   The virtual interrupt injection info registers of each virtual interrupt injection block. +

- *Virtual Interrupt Injection Control Registers* +
   The virtual interrupt injection control registers of each virtual interrupt injection block.
...
*PLIC Virtual Interrupt Injection Registers Memory Map* +
	...
	base + 0x400000: Virtual Interrupt Injection Registers 0 block 0
	base + 0x400004: Virtual Interrupt Injection Registers 1 block 0
	base + 0x400008: Virtual Interrupt Injection Registers 2 block 0
	...
	base + 0x40003C: Virtual Interrupt Injection Registers 15 block 0
	base + 0x400040: Virtual Interrupt Injection Registers 0 block 0
	base + 0x400044: Virtual Interrupt Injection Registers 1 block 1
	base + 0x400048: Virtual Interrupt Injection Registers 2 block 1 
	...
	base + 0x40004C: Virtual Interrupt Injection Registers 15 block 1
	...
	base + 0x400400: Virtual Interrupt Injection Registers 0 block 15 
	base + 0x400404: Virtual Interrupt Injection Registers 1 block 15
	base + 0x400408: Virtual Interrupt Injection Registers 2 block 15 
	...
	base + 0x40043C: Virtual Interrupt Injection Registers 15 block 15 
...
PLIC has a 32-bit width memory mapped register for each injection block to provide the hypervisor a mean to infer which `injection registers` are available for delivering guest virtual interrupts.

The `virtual Interrupt injection status registers` is a read-only register. Where each bit represents the injection register state :

- *1 -* Injection register available.
- *0 -* Injection register not available.
...
| *PLIC Register Block Name* | *Function*|*Register Block Size in Byte*| *Description*
|Virtual Interrupt Injection Status Registers
|Virtual Interrupt Injection Status Registers from Block #0 to #15 for 15872 contexts
|4 * 16 = 64 (0x000100) bytes
|This is a continuously memory block contains Virtual Interrupt Injection Status Bits for each injection block. Each Virtual Interrupt  Injection Status occupies 1-bit from this register block.
|===

*PLIC Virtual Interrupt Injection Status Registers Memory Map* +

	base + 0x400000: Interrupt Injection Status Register block 0
	base + 0x400004: Interrupt Injection Status Register block 1
	base + 0x400008: Interrupt Injection Status Register block 2
	...
	base + 0x400400: Interrupt Injection Status Register block 15 

== Virtual Interrupt Injection Info Registers

PLIC has a 32-bit width memory mapped register for each injection block to provide the hypervisor information about how many list registers are implemented on a certain block and the maximum priority levels.

The `virtual Interrupt injection status registers` is a read-only register.
...
|This is a continuously memory block contains PLIC Virtual Interrupt Pending Bits for each guest. Each Virtual Interrupt Pending Bit occupies 1-bit from this register block.
|===
*PLIC Virtual Interrupt Injection Info Registers Memory Map* +
...
PLIC has a 32-bit width memory mapped control register for each injection block where the hypervisor can configure.

There are 2 fields: `enable` field to configure which injection registers are enabled, 1-bit for each injection register. A `guestcontxid` to configure which context does the block influence.  

The `virtual Interrupt injection control registers` are a read-write register. Only the hypervisor can acess these registers. 
...
*PLIC Virtual Interrupt Injection Control Registers Memory Map* +

	base + 0x400000: Virtual Interrupt Injection Control block 0
	base + 0x400004: Virtual Interrupt Injection Control block 1
	base + 0x400008: Virtual Interrupt Injection Control block 2
	...
	base + 0x40003C: Virtual Interrupt Injection Control block 15
