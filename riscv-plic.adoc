...
- *Virtual Interrupt Virtual Block ID registers:* +
   The virtual interrupt block id of each context. +

- *Virtual Interrupt Injection Registers:* +
   The virtual interrupt injection registers for each injection block . +
...
*PLIC Memory Map*
	...
	base + 0x3FFFFFC: Reserved	
	...
	base + 0x4000000: Block ID Register bit 0-31 context 0
	base + 0x4000004: Block ID Register bit 0-31 context 1
	base + 0x4000008: Block ID Register bit 0-31 context 2
	...
	base + 0x400F7FC: Block ID Register bit 0-31 context 15871
	...
	base + 0x4010000: Virtual Interrupt Injection Registers 0 block 1
	base + 0x4010004: Virtual Interrupt Injection Registers 1 block 1
	base + 0x4010008: Virtual Interrupt Injection Registers 2 block 1
	...
	base + 0x4010FFC: Virtual Interrupt Injection Registers 1023 block 1
	base + 0x4011000: Virtual Interrupt Injection Registers 0 block 2
	base + 0x4011004: Virtual Interrupt Injection Registers 1 block 2
	base + 0x4011008: Virtual Interrupt Injection Registers 2 block 2 
	...
	base + 0x4011FFC: Virtual Interrupt Injection Registers 1023 block 2
	...
	base + 0x40FF000: Virtual Interrupt Injection Registers 0 block 240
	base + 0x40FF004: Virtual Interrupt Injection Registers 1 block 240
	base + 0x40FF008: Virtual Interrupt Injection Registers 2 block 240 
	...
	base + 0x40FFFFC: Virtual Interrupt Injection Registers 1023 block 240
	...	
...
== Virtual Context Block ID Register 

Each context has one register to specify the block source for virtual interrupt injection.

Setting this field zero indicates that no injection block is attached to context.

[cols="15%,20%,20%,45%"]
|===
| *PLIC Register Block Name* | *Function*|*Register Block Size in Byte*| *Description*
|Virtual Context Block ID Register
|Virtual Context Block ID Register for each context
|4 * 15872 = 63488(0xf800) bytes
|This is the Virtual Context Block ID Register for each context.
|===
...
== Virtual Interrupt Claim/Complete Process

When a claim is performed, the `inFlight` bit is set indicating that the interrupt is active.

When a complete is performed, the interrupt `int_id` field is set to zero and the `inFlight` bit is cleared. 

== Virtual Interrupt Injection Registers 

PLIC has a 240 memory-mapped blocks with 32-bit width registers to inject virtual interrupts into any context. Each registers has 4 different fields, 10 bits `intId` field to specify the interrupt source, 1 bit `inFLight` to state the interrupt state and 10 bits `priority` field to specify the interrupt priority. The hypervisor controls the state of this registers for each context. 

A virtual interrupt is pending when field `intid > 0` and the `inFlight` is not set.

The bit `inFlight` is set when the virtual interrupt is pending and a claim is performed, indicating that the interrupt is active and preventing the virtual interrupt from being pending.

The `injection block` is a continuously memory block with 4KB (page size) with 1024 maximum number of injection registers.

PLIC implementations with virtualization support must implement at least one injection register for each block.
...
| *PLIC Register Block Name* | *Function*|*Register Block Size in Byte*| *Description*
|Virtual Interrupt Injection Registers
|Virtual Interrupt Injection Registers from Block #0 to #240 for 15872 contexts
|240 * 4096 = 983040 (0xf0000) bytes for each block
|This is a continuously memory block contains PLIC Virtual Interrupt Injection Registers for each block. Each Injection Registers occupies 32-bit from the register block and total 240 Injection Blocks. Each block ocupies 4KB (page size). 
|===
